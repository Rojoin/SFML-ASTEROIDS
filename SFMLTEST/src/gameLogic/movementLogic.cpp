...
namespace GameLogic
{
	...
	{
		Vector2f mouse = Inputs::getMouseInput();
		Vector2f shipPos = { ship.dest.x, ship.dest.y };
		Vector2f direction = { mouse.x - shipPos.x, mouse.y - shipPos.y };
		return direction;
	}
	...
	void moveSpaceShip(SpaceShip& ship)
	{
		...
		Vector2f normalizedDirection = Vector2fNormalize(ship.direction);
	...
	{
		Vector2f direction = { asteroid.circle.position.x - GameLogic::currentShipPos.x, asteroid.circle.position.y - GameLogic::currentShipPos.y };
		float grades = (atan(direction.y / direction.x)) * (180 / PI);
		if (direction.x < 0)
		{
			grades += 180;
